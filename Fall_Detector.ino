...
#define READINGS 10 // number of readings taken between start of the fall and impact with the ground
...
void loop(void) 
{
  ...
  {
    int i;
    for(i=0;i<READINGS;i++) // Get readings after the lowerThresholdAccel is broken
    {
      accelReading = magnitudeAccel();
      ...
      if ((accelReading > upperThresholdAccel) && (gyroReading > thresholdGyro))  // check each of the new readings to see if the user is falling
      {
