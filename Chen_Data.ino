...
double thresholdAcc()
{
  ...
  xsquared = (accelerometer.x())*(accelerometer.x());
  ysquared = (accelerometer.y())*(accelerometer.y());
  zsquared = (accelerometer.z())*(accelerometer.z());
...
// if acc threshold value is attained over 0.5 second, send in squared gyroscope and accelerometer value to process
  if ( squaredAccValue > thresholdAccel )
  {
    counter++;
    if ( counter > 5 )
    {
